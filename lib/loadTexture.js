...
function loadTexture(texturePath, options) {
  ...
  return fsExtra.readFile(texturePath).then(function (source) {
    ...
    // options.writer(texturePath, texture.source);
    ...
    if (extension === ".png") {
      ...
      decodePromise = decodeTga(texture, options);
    }

    // console.log(decodePromise);
...
function decodeTga(texture, options) {
  const filepath = texture.path;
  const name = path.basename(filepath, path.extname(filepath));
  const savePath = path.join(path.dirname(filepath), name + ".png");
  return tga2png(filepath, savePath).then((buf) => {
    texture.source = buf;
    texture.name = path.basename(savePath, path.extname(savePath));
    texture.extension = path.extname(savePath).toLowerCase();
