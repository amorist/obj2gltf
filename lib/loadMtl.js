 ...
 * @private
 */
function loadMtl(mtlPath, objDirectory, options) {
  let material;
  let values;
  let value;
...
function loadMaterialTexture(
  ...
  const files = genFileList(objDirectory);
  const fileMap = {};
  files.forEach(function (file) {
    fileMap[path.basename(file).toLowerCase()] = file;
  });
  const shallowPath = fileMap[path.basename(texturePath).toLowerCase()];
...
function createDiffuseAlphaTexture(diffuseTexture, alphaTexture, options) {
  ...
  texture.name = diffuseTexture.name + "_diffuse_alpha";
...
function readFile(filepath, filesList) {
  const files = fs.readdirSync(filepath); // 需要用到同步读取
  files.forEach((file) => {
